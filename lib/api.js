...
export async function loginCashier(mobile, loginCode) {
  ...
  const res = await fetch(`${API_BASE}/api/cash/auth/login`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ mobile_number: mobile, login_code: loginCode  ,  latitude: DEFAULT_LATITUDE,
      longitude:  DEFAULT_LONGITUDE, }),
...
function normalizeBill(b) {
  return {
    ...
    whatsappSent: b.whatsapp_sent,
    itemCount: b.item_count,
    numberOfCartoon: b.number_of_cartoon,
...
// PATCH /api/sales/bills/:id  — update customer, GSTIN, discount, round-off, etc.
export async function updateBill(id, payload) {
  const res = await authedFetch(`/api/admin/bills/${id}`, {
    method: "PATCH",
